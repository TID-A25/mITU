...
export default function useProfiles({ excludeUserId, currentUserId } = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Exclusion happens inside fetchProfiles
        ...
        if (currentUserId) {
          try {
            const userInterests = await fetchCurrentUserInterests(
              currentUserId
            );
            if (!mounted) return;
  ...
  // Callback: refresh profiles by incrementing counter which triggers refresh to re-fetch
