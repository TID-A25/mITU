...
export default function useProfile(profileId, { autoRefresh = true } = {}) {
  ...
  const handleUserChange = useCallback((newUserId) => {
    ...
    refresh(); // refetches the profile data for the new user
  }, []);

  //Load saved user from localstorage and copy it into state
  useEffect(() => {
    const init = async () => {
  ...
  useEffect(() => {
    if (!profileId) return;
    // When autoRefresh is false, don't fetch before refresh() is called
    ...
    return () => {
      mounted = false; // set mounted to false so the async code won’t try to update state.
    };
  }, [profileId, autoRefresh, refreshIndex]);

  //increments index by 1 which triggers refreshIndex useEffect which re-fetches profile
  ...
  // the below function is not called anywhere in the codebase.
  const updateLocal = useCallback(
    (patch) => setProfile((p) => (p ? { ...p, ...patch } : p)),
    []
  );
